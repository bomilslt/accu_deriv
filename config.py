...
SYMBOL = os.getenv("DERIV_SYMBOL","R_100") # Volatility 100 (1s) - Ajuster selon besoin (R_10, R_25, etc.)
INITIAL_STAKE = 10.0 # Mise initiale en USD

ACCOUNT_TYPE = os.getenv("DERIV_ACCOUNT_TYPE", "demo") # "demo" ou "real" - compte à utiliser (démo par défaut, plus sûr)
...
TARGET_TICKS_MIN = int(os.getenv("TARGET_MIN", "4")) # Option B: Sortie automatique après min 4 ticks
TARGET_TICKS_MAX = int(os.getenv("TARGET_MAX", "5")) # Option B: Sortie automatique après max 5 ticks (si TP pas atteint avant)
...
ABNORMAL_MOVE_THRESHOLD = float(os.getenv("ABNORMAL_MOVE_THRESHOLD", "3.0"))
...
TREND_FILTER_ENABLED = os.getenv("TREND_FILTER_ENABLED", "True").lower() in ("true", "1", "yes")   # Activer le filtre de tendance avant chaque achat
TREND_WINDOW = 20             # Nb de ticks analysés pour le momentum directionnel
TREND_DIRECTIONALITY = float(os.getenv("TREND_DIRECTIONALITY", "0.75"))    # Fraction de ticks dans une même direction requise (0.5-1.0)
...
REOBSERVE_TICKS = int(os.getenv("REOBSERVE_TICKS", "30"))        # ticks frais à collecter après une clôture avant d'entrer
CALM_CONFIRM_TICKS = int(os.getenv("CALM_CONFIRM_TICKS", "5"))      # ticks consécutifs "calmes" requis avant l'entrée
CALM_MAX_TICK_MOVE = float(os.getenv("CALM_MAX_TICK_MOVE", "0.0"))  # variat. max par tick jugée "calme" (0 = auto: barrière la plus large)
